        ...
        <div class="col-md-12">
            <div class="text-center mb-5">
                ...
                <h3 class="mt-3 text-center mb-3 font-size-18">
                    <p>
                        <span class="d-none" id="host-waiting-message"><?= $lang('waiting_for_host_to_start_this_session') ?></span>
                        <div>
                            <img src="<?= URL::asset('\Application\Assets\images\loader.gif') ?>" class="img-fluid" id="loader" width="25px" height="25px" alt="">
                        </div>
                    </p>
                    <p id="time-remaining"><?= $lang('time_remaining_to_start_session')?></p>
                    <div class="countdown" style="margin-top: 0px;"></div>
                    <p id="advisor-late-message" class="d-none"><?= $lang('waiting_for_the_host_to_start_the_session')?></p>
...
<define footer_js>
    <script>
        var workshopId = "<?= $id ?>",
            ...
            joinUrl = isAdvisor ? workshopStartAndJoinURL : workshopJoinURL;
        ...
        countDown(new Date(workshopDate), function() {
            if (isAdvisor) {
                joinSession()
            }
        });

        var diff = new Date(workshopDate) - new Date();
        ...
        var channel = pusher.subscribe(`notifications.workshop.${workshopId}`);
        channel.bind('workshop_started', function (e) {
            joinSession()
        });
    </script>
</define>